...
class Explorer(Problem):
    ...
    def successor(self, state):
        ...
        """
        successors={}
        rows_columns=state[0]
        man=list(state[1])
        house=state[2]
        obstacles=state[3]
        if self.check_valid((man[0]+2, man[1]), rows_columns, obstacles) and (man[0]+1, man[1]) not in obstacles:
            new_state_man=(man[0]+2, man[1])
            successors["Desno 2"]=(rows_columns, new_state_man,house,obstacles)
        if self.check_valid((man[0]+3, man[1]), rows_columns, obstacles) and (man[0]+1, man[1]) not in obstacles and (man[0]+2, man[1]) not in obstacles:
            new_state_man=(man[0]+3, man[1])
            successors["Desno 3"]=(rows_columns, new_state_man,house,obstacles)
        if self.check_valid((man[0], man[1]+1), rows_columns, obstacles):
            new_state_man=(man[0], man[1]+1)
            successors["Gore"]=(rows_columns, new_state_man,house,obstacles)
        if self.check_valid((man[0], man[1]-1), rows_columns, obstacles):
            new_state_man=(man[0], man[1]-1)
            successors["Dolu"]=(rows_columns, new_state_man,house,obstacles)
        if self.check_valid((man[0]-1, man[1]), rows_columns, obstacles):
            new_state_man=(man[0]-1, man[1])
            successors["Levo"]=(rows_columns, new_state_man,house,obstacles)
        return successors
    def check_valid(self, coordinates, board, obstacles):
        x=coordinates[0]
        y=coordinates[1]
        if 0<=x<board and 0<=y<board and (x,y) not in obstacles:
            return True
        return False
...
if __name__ == '__main__':
    ...
    state=(n, tuple(man), tuple(house), tuple(obstacles))
